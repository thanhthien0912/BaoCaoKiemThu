# Product Context: Nền tảng Ví điện tử Sinh viên
...
- **Tích hợp MoMo**: Cổng thanh toán điện tử phổ biến nhất Việt Nam
...
- **MoMo Integration**: Tích hợp hoàn chỉnh với cổng thanh toán MoMo
  - Payment creation và status checking
  - Webhook handling với signature verification
  - Error mapping và retry logic
- **AI Chat**: Chatbot tư vấn tài chính với Google Gemini API
  - Context-aware: Biết đầy đủ thông tin chi tiêu, ví, giao dịch
  - Smart financial advisor: Phân tích chi tiêu và đưa ra gợi ý
  - Vietnamese conversational interface
  - Rate limiting: 20 messages/minute
- **NFC Support**: API cho đọc/ghi thẻ NFC
- **Admin Features**: Quản lý người dùng và giao dịch

### Frontend Web
- **Student Interface**: Dashboard, wallet, transaction history
- **Admin Dashboard**: Thống kê, quản lý sinh viên, giao dịch
- **Material-UI**: Giao diện hiện đại và responsive
- **Role-based Access**: Phân quyền theo vai trò

### Mobile App (Flutter)
- **Cross-platform**: Hỗ trợ cả Android và iOS
- **NFC Integration**: Đọc/ghi thẻ NFC
- **MoMo Payment**: Tích hợp thanh toán qua MoMo
- **Secure Storage**: Lưu trữ token an toàn
- **Riverpod State Management**: Quản lý state hiệu quả
